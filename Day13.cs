...
public partial class Day13 : IPuzzle
{
    ...
    public void LoadInput(string inputPath)
    {
        ...
        {
            var xys = lines.Select(l => matchInt.Matches(l).Select(m => long.Parse(m.Value))).SelectMany(n => n).ToArray();
    ...
    public long Part2()
    {
        equations = equations.Select(eq => new Equation(eq.XA, eq.YA, eq.XB, eq.YB, eq.XT + 10000000000000, eq.YT + 10000000000000));
        return equations.Select(Solve).Sum(s => s.a * 3 + s.b);
    }
}
